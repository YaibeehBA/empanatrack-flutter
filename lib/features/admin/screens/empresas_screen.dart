...
import '../../../core/utils/ubicacion_util.dart';
...
class EmpresasScreen extends ConsumerWidget {
                ...
                sliver: SliverList(
                  delegate: SliverChildBuilderDelegate(
                    (ctx, i) => _EmpresaCard(
                      ...
                      onEliminar: () => _confirmarEliminar(ctx, ref, lista[i]),
                    ),
  ...
  void _confirmarEliminar(BuildContext context, WidgetRef ref, EmpresaAdmin empresa) {
    showDialog(
      context: context,
  ...
  static void _mostrarSnack(BuildContext ctx, String msg, {bool isError = false}) {
    ScaffoldMessenger.of(ctx).showSnackBar(
      SnackBar(
        ...
        backgroundColor: isError ? AppColores.danger : AppColores.success,
  ...
  Widget build(BuildContext context) => const Center(
        child: Column(
          ...
          children: [
            ...
            SizedBox(height: 16),
            Text('Cargando empresas…', style: TextStyle(color: AppColores.textSecond)),
          ],
        ),
      );
}
        ...
        child: Column(
          ...
          children: [
            ...
            const Text('No pudimos cargar las empresas',
                style: TextStyle(fontWeight: FontWeight.w600, color: AppColores.textPrimary)),
            const SizedBox(height: 6),
            const Text('Verifica tu conexión e intenta de nuevo.',
                style: TextStyle(fontSize: 13, color: AppColores.textSecond)),
          ],
        ),
      );
}
        ...
        child: Column(
          ...
          children: [
            ...
            const Text('Toca "+ Nueva Empresa" para comenzar.',
                style: TextStyle(fontSize: 14, color: AppColores.textSecond)),
          ],
        ),
      );
}
...
class _EmpresaCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final tieneGPS = empresa.latitud != null && empresa.longitud != null;
                          ...
                          children: [
                            Row(
                              ...
                              children: [
                                ...
                                if (!empresa.estaActiva) _BadgeInactiva(),
                              ],
                            ),
...
class _GpsChip extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) => Row(children: [
        const Icon(Icons.my_location_rounded, size: 13, color: AppColores.success),
        const SizedBox(width: 4),
        Text(
...
class _SinGps extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) => Row(children: [
        Icon(Icons.location_off_outlined, size: 13, color: Colors.grey.shade400),
        const SizedBox(width: 4),
        Text('Sin ubicación GPS', style: TextStyle(fontSize: 11, color: Colors.grey.shade400)),
      ]);
}
...
class _DialogEliminar extends StatelessWidget {
  ...
  const _DialogEliminar({required this.empresa, required this.onConfirmar});
          ...
          child: Column(
            ...
            children: [
              Container(
                ...
                child: const Center(
                  child: Icon(Icons.delete_forever_rounded, color: AppColores.danger, size: 30),
                ),
              ),
              ...
              RichText(
                ...
                text: TextSpan(
                  style: const TextStyle(color: AppColores.textSecond, fontSize: 14, height: 1.5),
                  children: [
              ...
              Container(
                ...
                decoration: BoxDecoration(
                  ...
                  border: Border.all(color: AppColores.warning.withOpacity(0.25)),
                ),
                child: const Row(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Icon(Icons.lightbulb_outline_rounded, color: AppColores.warning, size: 16),
                    SizedBox(width: 8),
                    Expanded(
                      child: Text(
                        'Si tiene clientes asociados, no se puede eliminar. '
                        'Desactívala desde "Editar" en su lugar.',
                        style: TextStyle(fontSize: 12, color: AppColores.warning, height: 1.4),
                      ),
                    ),
                  ],
                ),
...
//  FORMULARIO EMPRESA
// ══════════════════════════════════════════════════════════
enum GpsInputMethod { currentLocation, importMaps, manual }
...
class _FormEmpresaState extends ConsumerState<_FormEmpresa> {
  late final _nombreCtrl = TextEditingController(text: widget.empresa?.nombre);
  late final _dirCtrl = TextEditingController(text: widget.empresa?.direccion);
  late final _teleCtrl = TextEditingController(text: widget.empresa?.telefono);
  late final _latCtrl = TextEditingController(text: widget.empresa?.latitud?.toString() ?? '');
  late final _lngCtrl = TextEditingController(text: widget.empresa?.longitud?.toString() ?? '');
  ...
  // Control de método GPS seleccionado
  GpsInputMethod _selectedGpsMethod = GpsInputMethod.currentLocation;
  ...
  void initState() {
    ...
    if (widget.empresa != null) {
      ...
      if (lat.isNotEmpty && lng.isNotEmpty) {
        ...
        // Si ya tiene coordenadas, mostramos el método manual
        _selectedGpsMethod = GpsInputMethod.manual;
      }
    }
  }
  ...
  Color _fondo(bool? ok) {
    ...
    return ok ? AppColores.success.withOpacity(0.04) : AppColores.danger.withOpacity(0.04);
  }
    ...
    String? hint,
      ...
      InputDecoration(
        ...
        enabledBorder: OutlineInputBorder(
          borderRadius: BorderRadius.circular(14),
          borderSide: BorderSide(color: _borde(ok), width: ok != null ? 1.5 : 1),
        ),
        ...
        errorBorder: OutlineInputBorder(
          borderRadius: BorderRadius.circular(14),
          borderSide: const BorderSide(color: AppColores.danger, width: 1.5),
        ),
        focusedErrorBorder: OutlineInputBorder(
          borderRadius: BorderRadius.circular(14),
          borderSide: const BorderSide(color: AppColores.danger, width: 2),
        ),
  ...
  Future<void> _usarUbicacionActual() async {
    ...
    final pos = await UbicacionUtil.obtenerPosicionActual();
    ...
    setState(() {
      _latCtrl.text = pos.lat.toStringAsFixed(7);
      _lngCtrl.text = pos.lng.toStringAsFixed(7);
      _latitudOk = true;
      _longitudOk = true;
    });
    ...
    if (context.mounted) {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: const Row(children: [
          ...
          SizedBox(width: 10),
          Text('Ubicación obtenida correctamente', style: TextStyle(fontWeight: FontWeight.w500)),
        ]),
  ...
  Future<void> _parsearUrl() async {
    ...
    final resultado = await ref.read(adminOpProvider.notifier).parsearUrlMaps(url);
    ...
    if (resultado != null) {
      ...
      setState(() {
        ...
        if (_nombreCtrl.text.trim().isEmpty && nombre != null && (nombre as String).isNotEmpty) {
      ...
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          content: const Row(children: [
            Icon(Icons.check_circle_rounded, color: Colors.white, size: 18),
            SizedBox(width: 10),
            Text('¡Coordenadas extraídas correctamente!', style: TextStyle(fontWeight: FontWeight.w500)),
          ]),
          backgroundColor: AppColores.success,
          behavior: SnackBarBehavior.floating,
          shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)),
  ...
  Future<void> _guardar() async {
    ...
    await widget.onGuardar({
      ...
      'telefono': _teleCtrl.text.trim().isEmpty ? null : _teleCtrl.text.trim(),
      'latitud': _latCtrl.text.trim().isEmpty ? null : double.tryParse(_latCtrl.text.trim()),
      'longitud': _lngCtrl.text.trim().isEmpty ? null : double.tryParse(_lngCtrl.text.trim()),
          ...
          Padding(
            ...
            child: Row(children: [
              Container(
                ...
                height: 38,
                decoration: BoxDecoration(
                  color: (esEdicion ? AppColores.primary : AppColores.warning).withOpacity(0.1),
                  borderRadius: BorderRadius.circular(11),
                ),
                child: Icon(
                  esEdicion ? Icons.edit_rounded : Icons.add_business_rounded,
                  color: esEdicion ? AppColores.primary : AppColores.warning,
                  size: 20,
                ),
              ),
              const SizedBox(width: 12),
              Column(
                ...
                children: [
                  ...
                  Text(
                    esEdicion ? 'Modifica los datos de la empresa' : 'Completa la información básica',
            ...
            child: SingleChildScrollView(
              ...
              child: Form(
                ...
                child: Column(
                  ...
                  children: [
                    // Sección info básica
                    _SectionLabel(label: 'Información básica', icon: Icons.info_outline_rounded),
                    ...
                    // Sección GPS mejorada
                    const SizedBox(height: 20),
                    _SectionLabel(label: 'Ubicación GPS', icon: Icons.my_location_rounded),
                    const SizedBox(height: 12),
                    // Selector de método
                    Container(
                      decoration: BoxDecoration(
                      ...
                      child: Column(
                        children: [
                          _GpsMethodOption(
                            ...
                            subtitle: 'Obtén coordenadas GPS automáticamente',
                            icon: Icons.my_location_rounded,
                            isSelected: _selectedGpsMethod == GpsInputMethod.currentLocation,
                            onTap: () {
                              setState(() {
                                _selectedGpsMethod = GpsInputMethod.currentLocation;
                                _usarUbicacionActual();
                              });
                            },
                          ),
                          _GpsMethodOption(
                            title: 'Importar desde Google Maps',
                            subtitle: 'Pega el enlace compartido',
                            icon: Icons.map_rounded,
                            isSelected: _selectedGpsMethod == GpsInputMethod.importMaps,
                            onTap: () {
                              setState(() {
                                _selectedGpsMethod = GpsInputMethod.importMaps;
                              });
                            },
                          ),
                          _GpsMethodOption(
                            title: 'Ingresar coordenadas manualmente',
                            subtitle: 'Latitud y Longitud',
                            icon: Icons.edit_location_alt_rounded,
                            isSelected: _selectedGpsMethod == GpsInputMethod.manual,
                            onTap: () {
                              setState(() {
                                _selectedGpsMethod = GpsInputMethod.manual;
                              });
                    ...
                    if (_selectedGpsMethod == GpsInputMethod.importMaps) ...[
                      _UrlMapsBox(
                      ...
                      if (_latCtrl.text.isNotEmpty && _lngCtrl.text.isNotEmpty) ...[
                        ...
                        Container(
                          ...
                          child: Row(
                            children: [
                              const Icon(Icons.check_circle, color: AppColores.success, size: 18),
                    ...
                    ] else if (_selectedGpsMethod == GpsInputMethod.manual) ...[
                      Row(children: [
                        Expanded(
                          child: TextFormField(
                            controller: _latCtrl,
                    ...
                    if (esEdicion) ...[
                      ...
                      Container(
                        decoration: BoxDecoration(
                          ...
                          borderRadius: BorderRadius.circular(14),
                          border: Border.all(
                            color: _estaActiva ? AppColores.success.withOpacity(0.3) : Colors.grey.shade300,
                          ),
                        ),
                        child: SwitchListTile(
                          ...
                          title: Text(
                            _estaActiva ? 'Empresa activa' : 'Empresa inactiva',
                            style: TextStyle(
                              fontSize: 14,
                              fontWeight: FontWeight.w600,
                              color: _estaActiva ? AppColores.success : AppColores.textSecond,
                            ),
                          ),
                          subtitle: Text(
                            _estaActiva
                            ...
                            style: TextStyle(
                              fontSize: 12,
                              color: _estaActiva ? AppColores.success.withOpacity(0.8) : Colors.grey.shade400,
                            ),
                          ),
                    ...
                    SizedBox(
                      ...
                      child: ElevatedButton(
                        ...
                        style: ElevatedButton.styleFrom(
                          backgroundColor: esEdicion ? AppColores.primary : AppColores.warning,
                          foregroundColor: Colors.white,
                          elevation: 0,
                        ...
                        child: _cargando
                            ...
                            : Row(
                                ...
                                children: [
                                  Icon(esEdicion ? Icons.save_rounded : Icons.add_business_rounded, size: 20),
                                  const SizedBox(width: 8),
                                  Text(
...
class _GpsMethodOption extends StatelessWidget {
  final String title;
  ...
  Widget build(BuildContext context) {
    return Material(
      color: isSelected ? AppColores.primary.withOpacity(0.05) : Colors.transparent,
      child: InkWell(
        onTap: onTap,
        borderRadius: BorderRadius.circular(14),
        child: Container(
          ...
          child: Row(
            children: [
              Container(
                ...
                height: 40,
                decoration: BoxDecoration(
                  color: isSelected ? AppColores.primary.withOpacity(0.1) : Colors.grey.shade100,
                  borderRadius: BorderRadius.circular(10),
                ),
                child: Icon(
                  icon,
                  color: isSelected ? AppColores.primary : Colors.grey.shade600,
                  size: 22,
                ),
              ),
                  ...
                  children: [
                    Text(
                      ...
                      style: TextStyle(
                        fontWeight: FontWeight.w600,
                        fontSize: 14,
                        color: isSelected ? AppColores.primary : AppColores.textPrimary,
                      ),
                    ),
                    const SizedBox(height: 2),
                    Text(
              ...
              if (isSelected)
                Icon(Icons.check_circle, color: AppColores.primary, size: 20)
              else
                Container(
                  width: 20,
                  height: 20,
          ...
          children: [
            Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Container(
                ...
                child: const Icon(Icons.share_location_rounded, color: AppColores.primary, size: 18),
              ),
              const SizedBox(width: 10),
              const Expanded(
                child: Column(
              ...
              Expanded(
                child: TextField(
                  ...
                  decoration: InputDecoration(
                    ...
                    hintStyle: const TextStyle(fontSize: 12, color: AppColores.textSecond),
                    prefixIcon: const Icon(Icons.link_rounded, size: 18, color: AppColores.textSecond),
                    filled: true,
