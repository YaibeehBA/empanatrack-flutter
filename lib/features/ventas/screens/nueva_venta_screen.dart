  ...
  Widget build(BuildContext context) {
    ...
    ref.listen<NuevaVentaState>(provider, (prev, next) {
      if (next.exitoso) {
        ...
        // Cerrar la pantalla primero, LUEGO mostrar el SnackBar en la pantalla anterior
        if (context.mounted) {
          Navigator.of(context).pop(true);
          // Esperar a que el Navigator cierre antes de mostrar el SnackBar
    ...
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
          // ── Banner reserva ───────────────────────────────
          if (_esDesdeReserva) ...[
            _BannerReserva(cliente: widget.clienteInicial?.nombre ?? 'Cliente'),
          ...
          if (!_esDesdeReserva) ...[
            ...
            _SelectorTipo(
              ...
              onChange: (tipo) => ref.read(provider.notifier).cambiarTipo(tipo),
            ),
          ...
          if (state.tipo == 'credito' && !_esDesdeReserva) ...[
            ...
            clientesAsync.when(
              ...
              error: (e, _) => const Text('Error cargando clientes'),
              data: (clientes) => _SelectorCliente(
                clientes: clientes,
                seleccionado: state.clienteSelec,
                buscarCtrl: _buscarClienteCtrl,
                onSeleccionar: (c) =>
                    ref.read(provider.notifier).seleccionarCliente(c),
              ),
            ),
            const SizedBox(height: 24),
          ],
          ...
          if (_esDesdeReserva)
            ...
            productosAsync.when(
              ...
              data: (productos) => _ListaProductos(
                ...
                onChange: (id, delta) =>
                    ref.read(provider.notifier).cambiarCantidad(id, delta),
                onAgregar: (p) =>
                    ref.read(provider.notifier).agregarProducto(p),
              ),
            ),
          const SizedBox(height: 24),
          ...
          TextField(
            ...
            maxLines: 2,
            onChanged: (v) => ref.read(provider.notifier).actualizarNotas(v),
