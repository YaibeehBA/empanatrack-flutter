...
class PanelInferiorRuta extends StatelessWidget {
  ...
  String get _textoBoton {
    // Prioridad 1: Ruta completamente finalizada (sesionCompletada o completada)
    if ((estado?.sesionCompletada ?? false) || (estado?.completada ?? false)) return 'Ver resumen del día';
    // Prioridad 2: Sin stock pero en ruta activa
  ...
  IconData get _iconoBoton {
    if ((estado?.sesionCompletada ?? false) || (estado?.completada ?? false)) return Icons.summarize_rounded;
      ...
      children: [
        ...
        SizedBox(
          ...
          child: ElevatedButton.icon(
            onPressed: cargando
                ? null
                : ((estado?.sesionCompletada ?? false) || (estado?.completada ?? false))
                    ? () {
                        if (onVerResumen != null) onVerResumen!();
                      }
                    : (sinStock && enRuta)
                        ? onFinalizarRuta
                        : enRuta
            ...
            style: ElevatedButton.styleFrom(
              backgroundColor: ((estado?.sesionCompletada ?? false) || (estado?.completada ?? false))
                  ? AppColores.primary
                  : (sinStock && enRuta)
                      ? AppColores.warning
                      : enRuta
  ...
  Widget build(BuildContext context) => Container(
    ...
    child: const Row(
      children: [
        ...
        Expanded(
          child: Text(
            'Has agotado tu stock del día. Puedes finalizar la ruta.',
